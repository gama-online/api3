...
FROM (
	SELECT account_number AS number, COALESCE(debit_currency, credit_currency) AS currency,
 	...
 	CROSS JOIN D
 	WHERE doc.company_id = :companyId AND doc.date = D.ob_date AND doc.archive IS NOT true AND doc.finished_gl AND account_number = :accountNumber
	...
	SELECT debit_number, amount_currency, 0, 0, SUM(amount_amount), 0, 0, 0
 	...
 	CROSS JOIN D
 	WHERE doc.company_id = :companyId AND doc.date > D.ob_date AND doc.date < :dateFrom AND doc.archive IS NOT true AND doc.finished_gl AND debit_number = :accountNumber
	...
	SELECT credit_number, amount_currency, 0, 0, 0, SUM(amount_amount), 0, 0
 	...
 	JOIN double_entries doc ON doc.id = op.parent_id
 	CROSS JOIN D
 	WHERE doc.company_id = :companyId AND doc.date > D.ob_date AND doc.date < :dateFrom AND doc.archive IS NOT true AND doc.finished_gl AND credit_number = :accountNumber
 	GROUP BY credit_number, amount_currency

	UNION
	SELECT debit_number, amount_currency AS currency, 0, 0, 0, 0, SUM(amount_amount), 0
 	FROM gl_operations op
 	JOIN double_entries doc ON doc.id = op.parent_id
 	WHERE doc.company_id = :companyId AND doc.date BETWEEN :dateFrom AND :dateTo AND doc.archive IS NOT true AND doc.finished_gl AND debit_number = :accountNumber
 	GROUP BY debit_number, amount_currency

	UNION
	SELECT credit_number, amount_currency AS currency, 0, 0, 0, 0, 0, SUM(amount_amount)
 	FROM gl_operations op
 	JOIN double_entries doc ON doc.id = op.parent_id
 	WHERE doc.company_id = :companyId AND doc.date BETWEEN :dateFrom AND :dateTo AND doc.archive IS NOT true AND doc.finished_gl AND credit_number = :accountNumber
 	GROUP BY credit_number, amount_currency
) AS A
