WITH D(ob_date) AS (
    ...
    SELECT part_id, P.name, sku,
    	packaging,
		COALESCE(G.registration_address_country, G.business_address_country, G.locations->0->>'country', 'LT') AS origin_country,
	...
	GROUP BY part_id, P.name, sku, packaging, origin_country
)
...
WHERE (ob_quantity <> 0 OR purchase_quantity <> 0 OR import_quantity <> 0
    ...
    AND (
	    ...
	    -- OR jsonb_exists(jsonb_path_query_array(packaging, '$[*].code'), :filter)
    )
ORDER BY lower(unaccent(name)), lower(unaccent(sku)), part_id, origin_country
